...
5. Make the module available, publicly.
    ```bash
    GOPROXY=proxy.golang.org go list -mutex "authentication-service/internal/library/example@v$(head VERSION)"
    ```
...
- Example: https://dev.go.dev/authentication-service/internal/library/example

Upon successful request, a message should be displayed:

> _We're still working on “authentication-service/internal/library/example”. Check back in a few minutes!_
