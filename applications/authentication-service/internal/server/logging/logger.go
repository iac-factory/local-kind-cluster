...
func caller() runtime.Frame {
	...
	for {
		...
		if !strings.HasPrefix(frame.Function, "authentication-service/internal/server") && !(strings.Contains(frame.Function, "log/slog")) {
			return frame
		}
		if !more {
			break
...
func (h *Handler) Handle(ctx context.Context, record slog.Record) error {
	...
	case levels.Notice:
		level = fmt.Sprintf("%s:", color.Color().Blue("NOTICE"))
	case levels.Warning:
