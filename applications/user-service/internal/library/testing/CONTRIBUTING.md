...
5. Make the module available, publicly.
    ```bash
    GOPROXY=proxy.golang.org go list -mutex "user-service/internal/library/example@v$(head VERSION)"
    ```
...
- Example: https://dev.go.dev/user-service/internal/library/example

Upon successful request, a message should be displayed:

> _We're still working on “user-service/internal/library/example”. Check back in a few minutes!_
