...
import (
	...
	"reconnaissance-service/internal/library/color"
...
func caller() runtime.Frame {
	...
	for {
		...
		if !strings.HasPrefix(frame.Function, "health-service/internal/library/server") && !(strings.Contains(frame.Function, "log/slog")) {
			return frame
		}
		if !more {
			break
...
func (h *Handler) Handle(ctx context.Context, record slog.Record) error {
	...
	case levels.Warning:
		level = fmt.Sprintf("%s:", color.Color().Yellow("WARNING"))
	case levels.Error:
		level = fmt.Sprintf("%s:", color.Color().Red("ERROR"))
	case levels.Emergency:
		level = fmt.Sprintf("%s:", color.Color().Red("EMERGENCY"))
	default:
