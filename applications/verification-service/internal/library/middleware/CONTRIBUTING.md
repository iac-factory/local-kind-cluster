...
5. Make the module available, publicly.
    ```bash
    GOPROXY=proxy.golang.org go list -mutex "verification-service/internal/library/example@v$(head VERSION)"
    ```
...
- Example: https://dev.go.dev/verification-service/internal/library/example

Upon successful request, a message should be displayed:

> _We're still working on “verification-service/internal/library/example”. Check back in a few minutes!_
